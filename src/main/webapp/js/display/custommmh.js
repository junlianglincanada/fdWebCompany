...
/*function traceKing(id,t){ 
    ...
    var traceSwitch = function(){
        ...
        var aText = _onUL.find("li:first h5").find("span").text(),
            ...
            cText = _onUL.find("li:first").find("span.date").text();  //console.log(aText,bText,cText) onmouseover="this.stop()" onmouseout="this.start()"
        var htmlWrap = '<marquee direction="left" behavior="slide" loop="1" scrollamount="80" width="100%" height="90" ><h5><span>'+aText+'</span></h5>'+
...
function traceKing(id,t){
    var _outDiv =$("#"+id),
        liW = _outDiv.find("li").width();
    var turnLeft = function(){
          _outDiv.find("li:last").clone(true).prependTo(_outDiv);
          _outDiv.find("li:first").fadeIn(1200).addClass("default").fadeIn(500).css("display","block");
          _outDiv.find("li").eq(1).removeClass("default");
          _outDiv.find("li").eq(1).find("h5 span").removeClass('first')
          _outDiv.find("li:first >div").css("marginLeft", liW +"px");
          _outDiv.find("li:first >div").animate({marginLeft: 0 + "px"}, 1500, "easeOutBounce", function(){
                _outDiv.find("li:last").remove();
                _outDiv.find("li.default h5 span").addClass('first');
          });
    }
    setInterval(turnLeft, t);
    return false;      
}
