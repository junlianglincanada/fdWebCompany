		...
		success:function(result){
			if(result.status==0){
				...
				for(var i=0;i<resultList.length;i++){
					...
					var companyName = isnull(resultList[i].companyName);
					var companyAddress = isnull(resultList[i].companyAddress);
					...
					content+='<span class="aq_td3">'+companyAddress+'</span>';
					content+='</dd>';
					var $tr=$(content);
...
$(function(){
	...
	$(window.parent.document).find("a.popup_closed").remove();
	$(window.parent.document).find(".popup_top").append($('<a href="javascript:void(0)" class="new_closed" title="关闭"><i class="closed"></i></a>'));
	$(window.parent.document).find("a.new_closed").live("click",function(){
		if(key==0){
