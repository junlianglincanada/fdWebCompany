...
function search(pageNum){
	...
	$("#loading").show();
	$.ajax({
	url: "../inputManage/inputBatch/queryInputBatchsData/"+pageNum+"/"+pageSize,
	type:"post",
    headers: { 
...
// 				var guarantee=isNullForTable(resultList[i].guaranteeValue)+""+isNullForTable(resultList[i].guaranteeUnitString);
				...
				var supplierName=isNullForTable(resultList[i].supplierName);
      			var $tr=$("<tr id="+id+"><td class='td_ser'>"+((pageNum-1)*pageSize+i+1)+"</td><td>"+inputDate+"</td><td >"+inputMatName+"</td><td>"+spec+"</td><td>"+manufacture+"</td><td>"+quantity+"</td><td>"+productionDate+"</td><td>"+productionBatch+"</td><td class='date'>"+guarantee+"</td><td>"+supplierName+"</td></tr>");
				$tr.data("id",id);
				if(i%2==0){
					$tr.addClass("even");
				}
...
/* 	//点击首页查询数据
	...
	$(".btn_export").click(function(){
		...
		$.ajax({
			url: "../inputManage/inputBatch/queryInputBatchsExportData",
			type:"post",
		    headers: { 
		        'Accept': 'application/json',
		        'Content-Type': 'application/json' 
...
<div id="loading"  style="position:absolute; top:50%; left:50%; margin:0 auto; height:300px; z-index: 888; display:none;"><img src="../img/loading.gif" ></div>
     <div class="main_con">
          <h3 class="process_title"><span>当前位置: </span><a href="../findView/replenish.parameter.history">历史进货查询</a> 
