...
function search(pageNum){
	...
	$("#loading").show();
	$.ajax({
	url: "../inputManage/inputBatch/queryImportFileList/"+pageNum+"/"+pageSize,
	type:"post",
    headers: { 
			...
			console.log(resultList);
			 for(var i=0;i<resultList.length;i++){
				...
				var fileName=resultList[i].fileName;
      			var $tr=$("<tr><td class='td_ser'>"+((pageNum-1)*pageSize+i+1)+"</td><td>"+uploadDate+"</td><td>"+fileName+"</td><td class='td_opera'><a href='../findView/replenish.parameter.parameter-detail?id="+id+"&importDate="+importDate+"&fileName="+encodeURI(encodeURI(fileName))+"' class='btn_green'>详细</a><a href='javascript:void(0)' class='btn_green' rel='popup' link='../findView/replenish.parameter.parameter-del?id="+id+"&importDate="+importDate+"&pageNum="+pageNum+"' title='提示'>批量删除台账</a></td></tr>");
				$tr.data("id",id);
				if(i%2==0){
					$tr.addClass("even");
				}
...
<div class="main_box">
     <div class="main_con">
          <h3 class="process_title"><span>当前位置: </span><a href="replenish.parameter.parameter?newSearch=1">近期进货维护</a> > <a href="#">按导入文件查看</a></h3>
