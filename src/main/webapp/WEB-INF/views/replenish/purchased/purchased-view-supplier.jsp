...
<div class="main_box">
         <div class="main_con">
			<h3 class="process_title"><span>当前位置: </span><a href="../findView/replenish.purchased.purchased?id=<%=id %>">采购品</a> > <a href="../findView/replenish.purchased.purchased-view?id=<%=id %>">查看采购品</a> > <a id="name1"></a> </h3>
          <div class="table_box">
                    <div class="info_tab">
               ...
               <a href="../findView/replenish.purchased.purchased-view-supplier?id=<%=id %>" class="default">供过货的供应商</a>
          </div>
               <table class="table_list" >
                    <thead>
                       <tr>
                           <th style="min-width:40px">序号</th>                 
                           <th style="min-width:100px">单位名称</th>
                           <th style="min-width:100px">单位地址</th>
                           <th style="min-width:100px">联系人</th>
                           <th style="min-width:250px">联系人电话</th>
                       </tr>
                    </thead>
                    <tbody >
                    </tbody>
...
function search(pageNum){
	...
	var toUrl="../inputManage/supplier/querySupplierByid/"+pageNum+"/"+pageSize;
	var inputMaterialId =id;
	console.log(inputMaterialId);
	params.inputMaterialId=inputMaterialId;
	$.ajax({
	url:toUrl,
	type:"post",
    headers: { 
...
function pageContentReset(result){
	if(result!=null&&result.body.resultList.length>0){
		...
		for(var i=0;i<resultList.length;i++){
			...
			var contactAddress=isNullForTable(resultList[i].contactAddress);
			var contactPerson=isNullForTable(resultList[i].contactPerson);
			var contactPhone=isNullForTable(resultList[i].contactPhone);
			var $tr=$('<tr><td>'+num+'</td><td>'+name+'</td><td>'+contactAddress+'</td><td>'+contactPerson+'</td><td>'+contactPhone+'</td></td></tr>');
			if(i%2==0){
				$tr.addClass("even");
			}
			$("tbody").append($tr);
