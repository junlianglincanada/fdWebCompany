	...
	$.ajax({
		...
		success:function(result){
			if(result.status==0){
				...
				if(linkCompanyId!=""){
					$("#inputProduct").attr("href","../findView/replenish.supplier.supplier-view-into?id=<%=id %>&linkCompanyId="+linkCompanyId);
				...
				if(foodProdCert==""&&foodCircuCert==""&&cateringCert==""&&bizCert==""&&foodBusinessCert==""){
...
<div class="main_box">
     <div class="main_con">
          <h3 class="process_title"><span>当前位置: </span><a href="replenish.supplier.supplier">供应商管理</a> > <a href="#">查看供应商</a> > <a id="name1"></a> 
              <div class="btn_opera"> 
                   <input type="button" class="btn_add" value="编辑供应商" onClick="window.location.href='replenish.supplier.supplier-editor2?id=<%=id %>'" />
              </div>
          </h3>
          <div class="info_tab">
               <a href="replenish.supplier.supplier-view?id=<%=id %>" class="default">基本信息</a>
               <a href="replenish.supplier.supplier-view-photo?id=<%=id%>" id="licenecImg">证照<em style="font-style:normal;position:relative;top:1px;letter-spacing:1px;">图</em>片</a>
           <a href="replenish.supplier.supplier-view-into?id=<%=id%>" id="inputProduct">关联的采购品<em style="font-style:normal;position:relative;top:1px;letter-spacing:1px;"></em></a>
          </div>
          <div class="info_box">
