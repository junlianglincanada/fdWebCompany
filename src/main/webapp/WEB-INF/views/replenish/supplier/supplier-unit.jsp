...
function search(pageNum){
	...
	$.ajax({
		url:"../inputManage/supplier/queryReceiverButNotSupplier/"+pageNum+"/"+pageSize,
		type:"post",
		...
		success:function(result){
			if(result.status==0){
				...
				if(list.length==0){
					$("#mainBody").hide();
					$("#can").load('replenish.supplier.supplier-nounit');
				}
...
function addCompany(){
	$.ajax({
		url:"../inputManage/supplier/addSuppliersByReceiverIds",
		type:"post",
...
$(function(){
	...
	$(".table_box ul.unit_list li").live("click",function(){
		...
		if(!$(this).hasClass("selected")){							  						  
			...
			var seltext = $(".table_box ul.unit_list li.selected a").text();
			var id=$(this).data("id");
			ids.push(id);
			addCompany();
		} else {
	...
	$(".btn_query").click(function(){
		...
		//设置全部参数IDS
		$.ajax({
			url:"../inputManage/supplier/queryReceiverButNotSupplier/1/-1",
			type:"post",
			data:{"companyName":companyName},
			dataType:"json",
			async: false,
...
<body>
<div id="can"></div>
<div class="main_box" id="mainBody">
     <div class="main_con">
          <h3 class="process_title"><span>当前位置: </span><a href="replenish.supplier.supplier">供应商管理</a> > <a href="#">维护协议单位</a> </h3>
          <div class="table_box">
               <h4 class="unit_title">以下单位将本企业设为<span class="blue">收货商</span>，是否将其添加为设为<span class="blue">供应商</span> </h4>
               <ul class="unit_list">                       
               </ul>
               <p class="paging_box">
